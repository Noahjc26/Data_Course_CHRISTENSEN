...
letters == "a"
letters != "c"
letters %ni% c("a","b","c","z")
vector1 %in% 1:6

'%ni%' <- Negate('%in%') #creates opposite function
...
dim(dat) #dimensions, numeric vector of length 2
(nrow(dat)) #number of rows
head(dat) #first 6 lines


# You can access specific columns of a "data frame" by name using '$'
dat$Species # $ isolates a specific column by name

levels(dat$Species)

dat$Sepal.Length

class(dat$Species) #factor vector
